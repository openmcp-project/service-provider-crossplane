...
// the Crossplane object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.21.0/pkg/reconcile
func (r *CrossplaneReconciler) Reconcile(ctx context.Context, _ ctrl.Request) (ctrl.Result, error) {
...
func (r *CrossplaneReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&crossplaneservicesopenmcpcloudv1alpha1.Crossplane{}).
		Named("crossplane").
		Complete(r)
}
