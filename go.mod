...
require (
	...
	ocm.software/ocm v0.41.0 // indirect
...
// ocm.software/ocm (transitive via control-plane-operator) requires both
// github.com/texttheater/golang-levenshtein v1.0.1 (parent pkg, contains levenshtein/ subdir)
// and this sub-pkg, which provides the same import path — causing an ambiguous import error.
// Excluding the sub-pkg forces Go to use the parent exclusively.
exclude github.com/texttheater/golang-levenshtein/levenshtein v0.0.0-20200805054039-cae8b0eaed6c
...
replace (
	...
	github.com/fluxcd/pkg/ssa => github.com/fluxcd/pkg/ssa v0.74.0

	// v0.35.x imports k8s.io/api/scheduling/v1alpha1 which was removed in k8s.io/api v0.36.
	// Upgrade to v0.36.1 which dropped that import.
